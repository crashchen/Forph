...
import type { FileInfo, RuntimeInfo } from "../lib/types";
...
export function DependencySection({
  ...
  const isPdfContext = fileType === "pdf";
  const isTranscriptionContext = fileType === "video" || fileType === "audio";
  const isBusy =
  ...
  const showModelCard =
    isTranscriptionContext &&
    runtime.whisper_available &&
    (!hasAnyModel || effectiveModel !== selectedModel);
  ...
  return (
    <>
      {isTranscriptionContext && !runtime.ffmpeg_available && (
      ...
      {isTranscriptionContext && !runtime.whisper_available && (
        <div className="mt-4 glass p-4 rounded-2xl text-left">
          <div className="flex items-start gap-3">
            <AlertTriangle size={16} className="text-warning shrink-0 mt-0.5" />
            <div className="space-y-2 w-full">
      ...
      {isPdfContext && !runtime.pdftotext_available && (
        <div className="mt-4 glass p-4 rounded-2xl text-left">
          <div className="flex items-start gap-3">
            <AlertTriangle size={16} className="text-warning shrink-0 mt-0.5" />
            <div className="space-y-2 w-full">
              <p className="text-sm font-medium text-white/78">
                先装 Poppler，才能提取 PDF 文字
              </p>
              <p className="text-xs text-white/42 leading-relaxed">
                当前 PDF 的文本提取依赖 `pdftotext`。装好后我会立刻帮你重新检测当前文件。
              </p>
              <div className="flex flex-wrap gap-2">
                {runtime.brew_available ? (
                  <button
                    disabled={isBusy}
                    onClick={() => onInstallDependency("poppler")}
                    className={`no-drag px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                      isBusy
                        ? "cursor-not-allowed bg-white/6 text-white/28"
                        : "cursor-pointer bg-accent/15 text-accent hover:bg-accent/25"
      ...
      {isTranscriptionContext && runtime.legacy_model_directories.length > 0 && (
      ...
      {isTranscriptionContext && runtime.available_models.length > 0 && (
